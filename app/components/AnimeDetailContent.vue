...
<template>
  <div class="min-h-screen relative overflow-hidden">
    <div class="absolute inset-0 z-0">
      <img :src="thumb(thumbnail, 32, 15)" alt="" width="32" height="43" loading="lazy" decoding="async" fetchpriority="low" class="w-full h-full object-cover scale-110 blur-3xl opacity-15 pointer-events-none">
    </div>
        ...
        <div class="flex flex-col lg:grid lg:grid-cols-[auto_1fr_minmax(280px,360px)] lg:gap-8 xl:gap-10 gap-5">
          <div class="flex gap-4 lg:block">
            <button type="button" class="flex-shrink-0 cursor-pointer rounded-lg overflow-hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50" @click="posterOpen = true">
              <img :src="thumb(thumbnail, 320, 30)" :alt="title" width="224" height="299" loading="eager" fetchpriority="high" decoding="async" sizes="(min-width: 1280px) 224px, (min-width: 1024px) 192px, (min-width: 640px) 160px, 128px" class="w-32 sm:w-40 lg:w-48 xl:w-56 rounded-lg shadow-2xl shadow-black/50 h-auto [filter:brightness(0.9)]">
            </button>
    ...
    <div v-if="posterOpen" data-tv-nav-scope class="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm" @click="closePoster">
      ...
      <div class="relative max-w-[90vw] max-h-[90vh]" @click.stop>
        <img :src="thumb(thumbnail, 400, 40)" :alt="title" width="400" height="533" loading="eager" decoding="async" class="max-h-[90vh] w-auto rounded-lg shadow-2xl object-contain [filter:brightness(0.9)]">
      </div>
    </div>
  </div>
</template>
