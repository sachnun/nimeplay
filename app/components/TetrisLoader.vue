...
function findLanding(shape: Shape, col: number): number {
  ...
  for (let t = 0; t < ROWS; t++) {
    ...
    for (const [dr, dc] of shape) {
      const r = t + dr
      const c = col + dc
      if (r >= ROWS || c < 0 || c >= COLS || grid.value[r]?.[c]) { ok = false; break }
    }
    if (ok) land = t
    else break
  }
...
function evaluate(sim: boolean[][]): number {
  ...
  let bumps = 0
  for (let c = 0; c < COLS - 1; c++) bumps += Math.abs((heights[c] ?? 0) - (heights[c + 1] ?? 0))
  return 8 * lines - 5 * holes - 2 * bumps - 0.3 * heights.reduce((a, b) => a + b, 0)
}

function pickBest(piece: Shape[]): { shape: Shape; col: number; land: number } | null {
  let best: { shape: Shape; col: number; land: number } | null = null
    ...
    if (full.length > 0) {
      ...
      schedule(() => {
        ...
        schedule(() => {
          ...
          for (let r = 0; r < ROWS; r++) {
            const row = grid.value[r]
            if (row && !full.includes(r)) kept.push([...row])
          }
