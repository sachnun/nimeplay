  ...
  <div class="absolute bottom-0 left-0 right-0 z-20 bg-gradient-to-t from-black/90 via-black/50 to-transparent transition-opacity duration-300 pointer-events-none" :class="controlsVisible ? 'opacity-100' : 'opacity-0'" :aria-disabled="disabled">
    <div class="px-4 md:px-8 pb-4 [@media_(hover:none)_and_(pointer:coarse)]:pb-[max(1rem,env(safe-area-inset-bottom))] pt-20 transition-opacity" :class="[controlsVisible && !disabled ? 'pointer-events-auto' : 'pointer-events-none', disabled ? 'opacity-50' : '']" :inert="disabled">
      ...
      <div class="flex items-center gap-1 md:gap-2">
        ...
        <div class="relative flex items-center" @mouseenter="$emit('showVolume')" @mouseleave="$emit('hideVolume')">
          <button class="w-9 h-9 flex items-center justify-center text-white/70 hover:text-white transition-colors cursor-pointer" aria-label="Mute" @click="$emit('toggleMute')">
          ...
          <div class="hidden md:flex items-center overflow-hidden transition-all duration-200" :class="showVolume ? 'w-20 opacity-100 ml-1' : 'w-0 opacity-0'">
            <input type="range" min="0" max="1" step="0.05" :value="isMuted ? 0 : volume" aria-label="Volume" class="vol-slider w-full h-1 appearance-none rounded-full cursor-pointer touch-none" :style="{ background: `linear-gradient(to right, #fff ${(isMuted ? 0 : volume) * 100}%, rgba(255,255,255,0.3) ${(isMuted ? 0 : volume) * 100}%)` }" @input="$emit('changeVolume', Number(($event.target as HTMLInputElement).value))">
          </div>
        </div>
