...
<template>
  <div data-tv-nav-scope class="absolute inset-0 z-40 flex items-center justify-center bg-black/90 backdrop-blur-sm">
    <div class="max-w-sm w-full mx-4 rounded-2xl bg-zinc-900 border border-white/10 shadow-2xl overflow-hidden">
      ...
      <div class="px-6 pb-6 flex gap-3">
        <NuxtLink :to="backTo" class="flex-1 py-2.5 text-sm font-medium text-center rounded-lg bg-white/5 border border-white/10 text-zinc-300 hover:bg-white/10 transition-colors">
          Kembali
        </NuxtLink>
        <button type="button" class="flex-1 py-2.5 text-sm font-medium rounded-lg bg-white text-zinc-900 hover:bg-zinc-200 transition-colors cursor-pointer" @click="$emit('continue')">
          Lanjutkan
        </button>
      </div>
    </div>
